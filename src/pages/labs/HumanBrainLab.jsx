...
import { ArrowLeft, Brain, RotateCcw, Maximize2, Layout, Map, Scissors } from 'lucide-react';

const HumanBrainLab = () => {
  const navigate = useNavigate();
  const iframeRef = React.useRef(null);
  const [isLoading, setIsLoading] = React.useState(true);
  const [currentView, setCurrentView] = React.useState('lobes');

  const handleReset = () => {
    if (iframeRef.current) {
      setIsLoading(true);
      iframeRef.current.src = iframeRef.current.src;
      setCurrentView('lobes');
    }
  };
  ...
  const changeView = (view) => {
    ...
    if (iframeRef.current && iframeRef.current.contentWindow) {
      iframeRef.current.contentWindow.postMessage({ type: 'SET_VIEW', view }, '*');
    }
  };
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#020617', color: '#f1f5f9', fontFamily: 'sans-serif', overflow: 'hidden' }}>
      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ lineHeight: 1.2 }}>
              ...
              <p style={{ fontSize: '10px', color: '#d946ef', fontWeight: 600, margin: 0 }}>Bộ não người (Human Brain)</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, justifyContent: 'flex-end' }}>
          ...
          <button 
            ...
            style={{ 
              display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', 
              backgroundColor: '#8b5cf6', border: 'none', borderRadius: '10px', 
      ...
      <main style={{ flex: 1, backgroundColor: '#020617', position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', height: '100%', maxWidth: '1200px', maxHeight: '800px', position: 'relative', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
          <iframe 
            ref={iframeRef}
            src="/external/native/human-brain/index.html" 
            onLoad={handleIframeLoad}
            style={{ width: '100%', height: '100%', border: 'none' }}
            title="Human Brain Simulation"
          />
        </div>

        {/* Decorative elements */}
        <div style={{ position: 'absolute', top: '20%', left: '5%', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(139, 92, 246, 0.05) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: '10%', right: '5%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(217, 70, 239, 0.05) 0%, transparent 70%)', pointerEvents: 'none' }} />
      </main>
      ...
      <div style={{ 
        ...
        <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: '#8b5cf6', boxShadow: '0 0 10px #8b5cf6', animation: 'pulse 2s infinite' }} />
